...
import type { AdapterModel } from "@agilo/adapter-utils";
import { asString, runChildProcess } from "@agilo/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AGILO_PI_COMMAND === "string" &&
    process.env.AGILO_PI_COMMAND.trim().length > 0
      ? process.env.AGILO_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AGILO_", "npm_", "NPM_"] as const;
