...
const AGILO_SESSIONS_DIR = path.join(os.homedir(), ".pi", "agilos");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[agilo] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[agilo] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[agilo] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(AGILO_SESSIONS_DIR, { recursive: true });
  return AGILO_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(AGILO_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readAgiloRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveAgiloDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.AGILO_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[agilo] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[agilo] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add Agilo skills directory so Pi can load the agilo skill
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[agilo] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
