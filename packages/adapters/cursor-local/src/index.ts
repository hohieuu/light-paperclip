...
Use when:
- You want Agilo to run Cursor Agent CLI locally as the agent runtime
...
- Agilo auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$agilo" and related skills on local runs.
- Agilo auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
