...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Agilo management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readAgiloRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureAgiloSkillSymlink(available.source, target);
  }
