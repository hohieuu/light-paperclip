...
## Scope
This plan is now **gateway-only**. Agilo supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Agilo skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Agilo task.
...
```bash
pnpm --filter @agilo/server test:run -- openclaw-gateway
pnpm --filter @agilo/server typecheck
pnpm --filter @agilo/ui typecheck
pnpm --filter agilo typecheck
```
