...
import { asNumber, asString, buildAgiloEnv, parseObject } from "@agilo/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "agilo";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "agilo";
  if (input.strategy === "run") return `agilo:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `agilo:issue:${input.issueId}`;
  return fallback;
}
...
function resolveAgiloApiUrlOverride(value: unknown): string | null {
...
function buildAgiloEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const agiloApiUrlOverride = resolveAgiloApiUrlOverride(ctx.config.agiloApiUrl);
  const agiloEnv: Record<string, string> = {
    ...buildAgiloEnv(ctx.agent),
    AGILO_RUN_ID: ctx.runId,
  };

  if (agiloApiUrlOverride) {
    agiloEnv.AGILO_API_URL = agiloApiUrlOverride;
  }
  if (wakePayload.taskId) agiloEnv.AGILO_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) agiloEnv.AGILO_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) agiloEnv.AGILO_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) agiloEnv.AGILO_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) agiloEnv.AGILO_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    agiloEnv.AGILO_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return agiloEnv;
}

function buildWakeText(payload: WakePayload, agiloEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/agilo-claimed-api-key.json";
  const orderedKeys = [
    "AGILO_RUN_ID",
    "AGILO_AGENT_ID",
    "AGILO_COMPANY_ID",
    ...
    "AGILO_TASK_ID",
    "AGILO_WAKE_REASON",
    "AGILO_WAKE_COMMENT_ID",
    "AGILO_APPROVAL_ID",
    "AGILO_APPROVAL_STATUS",
    "AGILO_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = agiloEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = agiloEnv.AGILO_API_URL ?? "<set AGILO_API_URL>";

  const lines = [
    "Agilo wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `AGILO_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AGILO_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AGILO_API_KEY on every API call.",
    "- Use X-Agilo-Run-Id: $AGILO_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AGILO_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AGILO_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$AGILO_COMPANY_ID/issues?assigneeAgentId=$AGILO_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardAgiloPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  agiloEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateAgilo = parseObject(payloadTemplate.agilo);
  const workspace = asRecord(ctx.context.agiloWorkspace);
  const workspaces = Array.isArray(ctx.context.agiloWorkspaces)
    ? ctx.context.agiloWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.agiloRuntimeServiceIntents)
    ? ctx.context.agiloRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardAgilo: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: agiloEnv.AGILO_API_URL ?? null,
  };

  if (workspace) {
    standardAgilo.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardAgilo.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardAgilo.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "agilo-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const agiloEnv = buildAgiloEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, agiloEnv);
  ...
  const agiloPayload = buildStandardAgiloPayload(ctx, wakePayload, agiloEnv, payloadTemplate);
