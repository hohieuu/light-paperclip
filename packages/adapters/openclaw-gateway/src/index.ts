...
Use when:
- You want Agilo to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Agilo server.
...
- agiloApiUrl (string, optional): absolute Agilo base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default agilo)

Standard outbound payload additions:
- agilo (object): standardized Agilo context added to every gateway agent request
- agilo.workspace (object, optional): resolved execution workspace for this run
- agilo.workspaces (array, optional): additional workspace hints Agilo exposed to the run
- agilo.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
