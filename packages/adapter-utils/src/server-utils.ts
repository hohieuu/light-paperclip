...
const AGILO_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface AgiloSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: AgiloSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "agilo_required",
      originLabel: "Required by Agilo",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Agilo",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "AGILO_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildAgiloEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.AGILO_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.AGILO_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.AGILO_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.AGILO_API_URL = apiUrl;
  return vars;
}
...
export async function resolveAgiloSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...AGILO_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listAgiloSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AgiloSkillEntry[]> {
  const root = await resolveAgiloSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Agilo skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Agilo cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredAgiloRuntimeSkills(value: unknown): AgiloSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: AgiloSkillEntry[] = [];
...
export async function readAgiloRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AgiloSkillEntry[]> {
  const configuredEntries = normalizeConfiguredAgiloRuntimeSkills(config.agiloRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listAgiloSkillEntries(moduleDir, additionalCandidates);
}

export async function readAgiloSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listAgiloSkillEntries(moduleDir);
...
export function readAgiloSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.agiloSkillSync;
...
function canonicalizeDesiredAgiloSkillReference(
  reference: string,
...
export function resolveAgiloDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readAgiloSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredAgiloSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeAgiloSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.agiloSkillSync;
  const current =
  ...
  next.agiloSkillSync = current;
  return next;
}

export async function ensureAgiloSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Agilo server itself is started from
    // within a Claude Code session (e.g. `npx agilo run` in a terminal
