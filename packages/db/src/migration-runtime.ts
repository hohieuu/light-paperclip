...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://agilo:agilo@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "agilo");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://agilo:agilo@127.0.0.1:${preferredPort}/agilo`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://agilo:agilo@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "agilo");
    return {
      connectionString: `postgres://agilo:agilo@127.0.0.1:${port}/agilo`,
  ...
  const adminConnectionString = `postgres://agilo:agilo@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "agilo");

  return {
    connectionString: `postgres://agilo:agilo@127.0.0.1:${selectedPort}/agilo`,
