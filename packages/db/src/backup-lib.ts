...
const STATEMENT_BREAKPOINT = "-- agilo statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900";
...
function formatSqlLiteral(value: string): string {
  ...
  while (sanitized.includes(tag)) {
    tag = `$agilo_${Math.random().toString(36).slice(2, 8)}$`;
  }
...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  const filenamePrefix = opts.filenamePrefix ?? "agilo";
