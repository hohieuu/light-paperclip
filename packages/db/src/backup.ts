...
function resolveAgiloInstanceId(): string {
  const raw = process.env.AGILO_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveAgiloHomeDir(), "instances", resolveAgiloInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveAgiloHomeDir(), "instances", resolveAgiloInstanceId(), "data", "backups");
}
