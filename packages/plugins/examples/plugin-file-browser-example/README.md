...
Example Agilo plugin that demonstrates:
...
```bash
pnpm --filter @agilo/plugin-file-browser-example build
pnpm agilo plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm agilo plugin uninstall agilo-file-browser-example --force
```
...
- Optional: use `agilo-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
