# @agilo/plugin-hello-world-example
...
- a minimal React widget rendered in the Agilo dashboard
...
```bash
pnpm --filter @agilo/plugin-hello-world-example build
pnpm agilo plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm agilo plugin uninstall agilo.hello-world-example --force` then  
  `pnpm agilo plugin install ./packages/plugins/examples/plugin-hello-world-example`.
