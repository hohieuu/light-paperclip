...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".agilo-sdk");
...
/**
 * Generate a complete Agilo plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Agilo plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "agilo-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["agilo", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@agilo/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@agilo/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@agilo/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@agilo/plugin-sdk\` and \`@agilo/shared\` from a local Agilo checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.agilo-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-agilo-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <agilo-sdk-path>]");
    process.exit(1);
  }
