/**
 * Core types for the Agilo plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@agilo/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @agilo/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Agilo object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Agilo UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Agilo domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Agilo domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Agilo secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Agilo secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@agilo/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@agilo/plugin-sdk";
 *
