...
import type { AgiloPluginManifestV1 } from "@agilo/shared";

import type { AgiloPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@agilo/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: AgiloPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@agilo/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
