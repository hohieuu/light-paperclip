...
Official TypeScript SDK for Agilo plugin authors.

- **Worker SDK:** `@agilo/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@agilo/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@agilo/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@agilo/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@agilo/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@agilo/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@agilo/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@agilo/plugin-sdk/ui/hooks` | Hooks only |
| `@agilo/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@agilo/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@agilo/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@agilo/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@agilo/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@agilo/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Agilo app. They can call ordinary Agilo HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Agilo instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@agilo/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@agilo/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Agilo information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@agilo/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@agilo/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@agilo/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@agilo/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@agilo/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@agilo/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@agilo/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@agilo/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@agilo/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@agilo/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@agilo/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@agilo/plugin-sdk/bundlers";
...
```bash
agilo-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@agilo/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__agilo__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__agilo__/events` streams `reload` SSE events on UI build changes
