...
- Agilo issue context now has incremental APIs: `GET /api/agents/me/inbox-lite`, `GET /api/issues/:id/heartbeat-context`, and comment delta queries via `GET /api/issues/:id/comments?after=...&order=asc`.
- The `agilo` skill now teaches agents to use those compact/incremental APIs first, while keeping full-thread fetches as a cold-start fallback.
...
- Runtime skill defaults now include `agilo`, `para-memory-files`, and `agilo-create-agent`. `create-agent-adapter` was moved to `.agents/skills/create-agent-adapter`.
