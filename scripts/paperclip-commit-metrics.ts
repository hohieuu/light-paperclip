...
const DEFAULT_QUERY = "\"Co-Authored-By: Agilo <noreply@agilo.tech>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/agilo-commit-metrics-cache.json");
...
const AGILO_EMAIL = "noreply@agilo.tech";
const AGILO_NAME = "agilo";
...
function printHelp() {
  console.log(`Usage: tsx scripts/agilo-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === AGILO_EMAIL) || (displayName && displayName.toLowerCase() === AGILO_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Agilo commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "agilo-commit-metrics",
