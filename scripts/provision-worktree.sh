...
base_cwd="${AGILO_WORKSPACE_BASE_CWD:?AGILO_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${AGILO_WORKSPACE_CWD:?AGILO_WORKSPACE_CWD is required}"
agilo_home="${AGILO_HOME:-$HOME/.agilo}"
agilo_instance_id="${AGILO_INSTANCE_ID:-default}"
agilo_dir="$worktree_cwd/.agilo"
worktree_config_path="$agilo_dir/config.json"
worktree_env_path="$agilo_dir/.env"
worktree_name="${AGILO_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${AGILO_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.agilo/config.json" || -L "$base_cwd/.agilo/config.json" ) ]]; then
  source_config_path="$base_cwd/.agilo/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$agilo_home/instances/$agilo_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm agilo --help >/dev/null 2>&1; then
    pnpm agilo worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v agilo >/dev/null 2>&1; then
    agilo worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  AGILO_WORKTREES_DIR="${AGILO_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const agiloDir = process.env.AGILO_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.AGILO_WORKTREES_DIR) ?? "~/.agilo-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(agiloDir, "config.json");
  const envPath = path.resolve(agiloDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "agilo",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.AGILO_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.AGILO_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.AGILO_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "AGILO_HOME=" + JSON.stringify(worktreeHome),
    "AGILO_INSTANCE_ID=" + JSON.stringify(instanceId),
    "AGILO_CONFIG=" + JSON.stringify(configPath),
    "AGILO_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "AGILO_IN_WORKTREE=true",
    "AGILO_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.AGILO_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("AGILO_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "agilo CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
