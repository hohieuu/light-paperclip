...
AGILO_HOST_PORT="${AGILO_HOST_PORT:-3100}"
AGILO_HOST_FROM_CONTAINER="${AGILO_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.agilo-openclaw.override.yml"
...
detect_agilo_base_url() {
  ...
  for candidate in "$AGILO_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${AGILO_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AGILO_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
agilo_base_url="$(detect_agilo_base_url || true)"
...
Agilo URL for OpenClaw container:
EOF
  if [[ -n "$agilo_base_url" ]]; then
    cat <<EOF
  $agilo_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AGILO_HOST_PORT}
  (Do not use http://127.0.0.1:${AGILO_HOST_PORT} inside the container.)
  If Agilo rejects the host, run on host machine:
    pnpm agilo allowed-hostname host.docker.internal
  Then restart Agilo and re-run this script.
EOF
  fi
  cat <<EOF
