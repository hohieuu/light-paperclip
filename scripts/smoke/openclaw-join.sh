...
COMPANY_ID="${COMPANY_ID:-${AGILO_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-agilo-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-agilo-openclaw-smoke}"
...
Provide one of:
  AGILO_AUTH_HEADER=\"Bearer <board-token>\"
  AGILO_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Agilo OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.agilo.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
