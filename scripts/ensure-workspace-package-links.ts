...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".agilo", "dist", "node_modules"]);
...
async function ensureServerWorkspaceLinksCurrent() {
  ...
  console.log("[agilo] detected stale workspace package links for server; relinking dependencies...");
  for (const mismatch of mismatches) {
    console.log(
      `[agilo]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
