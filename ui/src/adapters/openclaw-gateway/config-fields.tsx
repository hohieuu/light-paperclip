...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Agilo API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "agiloApiUrl",
                  String(config.agiloApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "agiloApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://agilo.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "agilo"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Agilo persists a device key during onboarding so pairing approvals
