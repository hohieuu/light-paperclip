 ...
 *
 * Plugin bundles are built with `external: ["@agilo/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__agiloPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__agiloPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__agiloPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__agiloPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@agilo/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@agilo/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agilo/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@agilo/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@agilo/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__agiloPluginBridge__) {
 ...
 * self-register via `window.agiloPlugins.registerReactComponent()`.
 ...
 * of `@agilo/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
