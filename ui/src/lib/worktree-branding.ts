...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("agilo-worktree-enabled") !== "true") return null;

  const name = readMetaContent("agilo-worktree-name");
  const color = normalizeHexColor(readMetaContent("agilo-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("agilo-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
