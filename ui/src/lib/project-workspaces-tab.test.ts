...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace } from "@agilo/shared";
...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace>): ExecutionWorkspace {
  return {
    ...
    name: overrides.name ?? "AGILO-893",
    status: overrides.status ?? "active",
    cwd: overrides.cwd ?? "/repo/.worktrees/AGILO-893",
    repoUrl: overrides.repoUrl ?? null,
    baseRef: overrides.baseRef ?? "public-gh/master",
    branchName: overrides.branchName ?? "AGILO-893-workspaces-tab",
    ...
    const summaries = buildProjectWorkspaceSummaries({
      ...
      executionWorkspaces: [
        createExecutionWorkspace({
          id: "exec-1",
          name: "AGILO-893",
          branchName: "AGILO-893-workspaces-tab",
