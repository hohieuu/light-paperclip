...
type AgiloSprite = (typeof AGILO_SPRITES)[number];
...
function spriteSize(sprite: AgiloSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AGILO_SPRITES[(baseCol + baseRow) % AGILO_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AGILO_SPRITES[Math.floor(Math.random() * AGILO_SPRITES.length)]!;
