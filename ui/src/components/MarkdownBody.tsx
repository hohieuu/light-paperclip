...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`agilo-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="agilo-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("agilo-mermaid-status", error && "agilo-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="agilo-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "agilo-mention-chip",
              `agilo-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "agilo-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "agilo-markdown prose prose-sm max-w-none break-words overflow-hidden",
