...
import { ensureAgentJwtSecret, loadAgiloEnvFile, mergeAgiloEnvEntries, readAgiloEnvEntries, resolveAgiloEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { AgiloConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printAgiloCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.AGILO_CONFIG;
...
const WORKTREE_NAME_PREFIX = "agilo-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.AGILO_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.AGILO_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromAgiloConfig(config: AgiloConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromAgiloConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.agilo${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".agilo", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".agilo", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.agilo"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: AgiloConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://agilo:agilo@127.0.0.1:${port}/agilo`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: AgiloConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.AGILO_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AGILO_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.AGILO_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AGILO_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: AgiloConfig;
  targetConfig: AgiloConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveAgiloEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readAgiloEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://agilo:agilo@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "agilo");
    const targetConnectionString = `postgres://agilo:agilo@127.0.0.1:${targetHandle.port}/agilo`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readAgiloEnvEntries(resolveAgiloEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.AGILO_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.AGILO_AGENT_JWT_SECRET);
  mergeAgiloEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { AGILO_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadAgiloEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Agilo inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printAgiloCliBanner();
  p.intro(pc.bgCyan(pc.black(" agilo worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printAgiloCliBanner();
  p.intro(pc.bgCyan(pc.black(" agilo worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasAgiloConfig: existsSync(path.resolve(worktreePath, ".agilo", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printAgiloCliBanner();
  p.intro(pc.bgCyan(pc.black(" agilo worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveAgiloEnvFile(configPath);
  const envEntries = readAgiloEnvEntries(envPath);
  const out = {
    AGILO_CONFIG: configPath,
    ...(envEntries.AGILO_HOME ? { AGILO_HOME: envEntries.AGILO_HOME } : {}),
    ...(envEntries.AGILO_INSTANCE_ID ? { AGILO_INSTANCE_ID: envEntries.AGILO_INSTANCE_ID } : {}),
    ...(envEntries.AGILO_CONTEXT ? { AGILO_CONTEXT: envEntries.AGILO_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasAgiloConfig)
      .map((choice) => path.resolve(choice.worktree, ".agilo", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readAgiloEnvEntries(resolveAgiloEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Agilo once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasAgiloConfig ? "agilo" : "no-agilo-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".agilo", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".agilo", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .agilo/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasAgiloConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Agilo worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasAgiloConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Agilo worktrees were found. Run `agilo worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Agilo configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Agilo instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Agilo instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with agilo- if needed (created at ~/agilo-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: AGILO_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: AGILO_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source AGILO_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Agilo instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Agilo worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with agilo- if needed")
