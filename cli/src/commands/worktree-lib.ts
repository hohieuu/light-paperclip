...
export const DEFAULT_WORKTREE_HOME = "~/.agilo-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".agilo");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "agilo",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    AGILO_HOME: paths.homeDir,
    AGILO_INSTANCE_ID: paths.instanceId,
    AGILO_CONFIG: paths.configPath,
    AGILO_CONTEXT: paths.contextPath,
    AGILO_IN_WORKTREE: "true",
    ...(branding?.name ? { AGILO_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { AGILO_WORKTREE_COLOR: branding.color } : {}),
  };
}
