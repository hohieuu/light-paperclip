...
function defaultConfig(): AgiloConfig {
  const instanceId = resolveAgiloInstanceId();
  return {
    $meta: {
      version: 1,
...
export async function configure(opts: {
  ...
  printAgiloCliBanner();
  p.intro(pc.bgCyan(pc.black(" agilo configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `agilo onboard` first.");
    p.outro("");
    return;
  }

  let config: AgiloConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because AGILO_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
