...
function buildAgentEnvExports(input: {
  ...
  return [
    `export AGILO_API_URL='${escaped(input.apiBase)}'`,
    `export AGILO_COMPANY_ID='${escaped(input.companyId)}'`,
    `export AGILO_AGENT_ID='${escaped(input.agentId)}'`,
    `export AGILO_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
    ...
    agent
      ...
      .action(async (opts: AgentListOptions) => {
        try {
          ...
          const rows = (await ctx.api.get<Agent[]>(`/api/agents`)) ?? [];
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Agilo skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Agilo skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveAgiloSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Agilo skills directory. Expected ./skills in the repo checkout.",
              );
            }
