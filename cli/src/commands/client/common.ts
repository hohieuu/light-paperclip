...
import { ApiRequestError, AgiloApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Agilo config file")
    .option("-d, --data-dir <path>", "Agilo data directory root (isolates state from ~/.agilo)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Agilo API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AGILO_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AGILO_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AGILO_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AGILO_COMPANY_ID, or set context profile companyId via `agilo context set`.",
    );
  }

  const api = new AgiloApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AGILO_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AGILO_SERVER_PORT || "");
