...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@agilo/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://agilo:agilo@127.0.0.1:${port}/agilo`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printAgiloCliBanner();
  p.intro(pc.bgCyan(pc.black(" agilo db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAgiloInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "agilo";
