...
export class AgiloApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-agilo-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Agilo server is not running, the configured URL is wrong, or the request is being blocked before it reaches Agilo.",
    "",
    "Try:",
    "- Start Agilo with `pnpm dev` or `pnpm agilo run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Agilo is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`AGILO_API_URL\`.`,
  );
  return lines.join("\n");
}
