...
const DATA_DIR_OPTION_HELP =
  "Agilo data directory root (isolates state from ~/.agilo)";

program
  .name("agilo")
  .description("Agilo CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadAgiloEnvFile(options.config);
});

program
  .command("doctor")
  .description("Run diagnostic checks on your Agilo setup")
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "agilo")
...
program
  .command("run")
  .description("Run Agilo with doctor checks")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Agilo server API")
