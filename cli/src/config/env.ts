...
const JWT_SECRET_ENV_KEY = "AGILO_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Agilo CLI commands",
...
export function resolveAgiloEnvFile(configPath?: string): string {
...
export function loadAgiloEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAgiloEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAgiloEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAgiloEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAgiloEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAgiloEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAgiloEnvEntries(next, filePath);
  return next;
}
