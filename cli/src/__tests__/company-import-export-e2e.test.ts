...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.AGILO_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.AGILO_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.AGILO_MIGRATION_AUTO_APPLY = "true";
  env.AGILO_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.AGILO_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.AGILO_MIGRATION_AUTO_APPLY;
  delete env.AGILO_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(args: string[], opts: { apiBase: string; configPath: string }) {
  ...
  const result = await execFileAsync(
    "pnpm",
    ["--silent", "agilo", ...args, "--api-base", opts.apiBase, "--config", opts.configPath, "--json"],
    {
      cwd: repoRoot,
      env: createCliEnv(),
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `agilo run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("agilo company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "agilo-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");

    tempDb = await startEmbeddedPostgresTestDatabase("agilo-company-cli-db-");
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    expect(readFileSync(path.join(exportDir, ".agilo.yaml"), "utf8")).toContain('schema: "agilo/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "agilo-demo"));
