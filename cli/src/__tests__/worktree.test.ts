...
function buildSourceConfig(): AgiloConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("agilo-pr-432")).toBe(
      path.resolve(os.homedir(), "agilo-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("agilo/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://agilo.example", 3110)).toBe("https://agilo.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/agilo-feature",
      homeDir: "/tmp/agilo-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/agilo-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/agilo-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.AGILO_HOME).toBe(path.resolve("/tmp/agilo-worktrees"));
    expect(env.AGILO_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.AGILO_IN_WORKTREE).toBe("true");
    expect(env.AGILO_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.AGILO_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export AGILO_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-secrets-"));
    const originalInlineMasterKey = process.env.AGILO_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.AGILO_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.AGILO_SECRETS_MASTER_KEY;
      delete process.env.AGILO_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.AGILO_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.AGILO_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.AGILO_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          AGILO_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.AGILO_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.AGILO_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".agilo-worktrees"),
      });

      const envPath = path.join(repoRoot, ".agilo", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("AGILO_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("AGILO_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/AGILO_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.AGILO_AGENT_JWT_SECRET;
      } else {
        process.env.AGILO_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".agilo-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".agilo", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Agilo config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".agilo", "config.json");
    const originalCwd = process.cwd();
    const originalAgiloConfig = process.env.AGILO_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalAgiloConfig === undefined) {
        delete process.env.AGILO_CONFIG;
      } else {
        process.env.AGILO_CONFIG = originalAgiloConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/agilo",
        targetRepoRoot: "/Users/example/agilo-pr-432",
        workspaceCwd: "/Users/example/agilo",
      }),
    ).toBe("/Users/example/agilo-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/agilo",
        targetRepoRoot: "/Users/example/agilo-pr-432",
        workspaceCwd: "/Users/example/agilo/packages/db",
      }),
    ).toBe("/Users/example/agilo-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agilo-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "agilo-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("agilo-make-test", {
        seed: false,
        home: path.join(tempRoot, ".agilo-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".agilo", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".agilo", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
