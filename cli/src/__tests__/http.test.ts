...
import { ApiConnectionError, ApiRequestError, AgiloApiClient } from "../client/http.js";

describe("AgiloApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AgiloApiClient({
    ...
    expect(headers["x-agilo-run-id"]).toBe("run-abc");
