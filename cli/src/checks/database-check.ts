...
export async function databaseCheck(config: AgiloConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `agilo configure --section database`",
      };
    }

    try {
      const { createDb } = await import("@agilo/db");
