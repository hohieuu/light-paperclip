...
Agilo V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Agilo-shaped design.

Assumption for this document: Agilo is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Agilo objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Agilo.

The main conclusion is:

- Agilo should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Agilo should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Agilo should use multiple extension classes instead of one generic plugin bag:
...
If Agilo does this well, the examples you listed become straightforward:
...
Relevant Agilo files reviewed for current extension seams:
...
It is too dangerous for Agilo core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Agilo — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Agilo should absolutely do this.
...
Agilo should copy this discipline.
...
Agilo should adopt this style for plugin actions, automations, and UI schemas.
...
Agilo can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Agilo too.

## What Agilo Should Not Copy Directly
...
Agilo is a control plane for an operator-managed instance with company objects.
...
Agilo is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Agilo should not auto-load arbitrary code from a workspace repo like `.agilo/plugins` or project directories.
...
For Agilo, equivalent hooks into:
...
Agilo should not allow plugins to silently replace:
...
For Agilo it would be risky because it combines:
...
Agilo should require an explicit operator install step.

## Why Agilo Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Agilo |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Agilo instance |
...
That means Agilo should borrow the good ideas from `opencode` but use a stricter architecture.

## Agilo Already Has Useful Pre-Plugin Seams

Agilo has several extension-like seams already:
...
This is good news.
Agilo does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Agilo Plugin Model
...
Agilo already has this pattern implicitly:
...
- `plugin` means integration code that talks to Agilo through a typed plugin protocol and capability model
...
- invoke explicit Agilo actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@agilo/plugin-sdk/ui`) with design tokens and common components.
...
Agilo should treat plugin installation as a global instance-level action.

Examples:

- install `@agilo/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Agilo objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Agilo already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Agilo too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@agilo/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@agilo/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Agilo instance.

A `create-agilo-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Agilo achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Agilo server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Agilo's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@agilo/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@agilo/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Agilo

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@agilo/plugin-sdk";

export default definePlugin({
  id: "@agilo/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Agilo entities
    });
    ...
    ctx.events.on("plugin.@agilo/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@agilo/plugin-sdk/ui";
...
Any Agilo plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Agilo objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@agilo/plugin-linear`)
...
- reuse core tables when the data is clearly part of Agilo itself
...
If a concept is becoming part of Agilo's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Agilo itself
...
### 3. Scope plugin data to Agilo objects before adding custom schemas

A lot of plugin data naturally hangs off existing Agilo objects:
...
If Agilo eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@agilo/plugin-workspace-files`
...
Package idea: `@agilo/plugin-terminal`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Agilo.
- Board reviews the diff after a run without leaving Agilo.
...
- optional `events.emit` (e.g. `plugin.@agilo/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@agilo/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@agilo/plugin-linear`

This plugin syncs Agilo work with Linear. It is useful for:

- importing backlog from Linear
- linking Agilo issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Agilo and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Agilo.
- Agilo issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@agilo/plugin-github-issues`

This plugin syncs Agilo issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Agilo issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Agilo task governance
...
- Plugin settings:
  ...
  - whether new Agilo issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Agilo issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Agilo.
- GitHub webhooks update status/comment state in Agilo.
- A PR is linked back to the Agilo issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@agilo/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@agilo/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Agilo. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Agilo issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Agilo plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Agilo dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Agilo issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Agilo core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@agilo/plugin-runtime-processes`
...
Package idea: `@agilo/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Agilo. It is useful for:
...
- Failed payment spikes or churn events can generate Agilo issues for follow-up.
...
- Stripe data should stay additive to Agilo core
...
- plugin UI bundle loading, host bridge, `@agilo/plugin-sdk/ui`
...
- `@agilo/plugin-test-harness` and `create-agilo-plugin` starter template
...
Agilo should not implement "an OpenCode-style generic in-process hook system."
Agilo should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Agilo
...
8. Ship `@agilo/plugin-test-harness` and `create-agilo-plugin` starter template.
