...
This guide describes the current, implemented way to create a Agilo plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Agilo app.
...
```bash
pnpm --filter @agilo/create-agilo-plugin build
node packages/plugins/create-agilo-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Agilo repo:

```bash
pnpm --filter @agilo/create-agilo-plugin build
node packages/plugins/create-agilo-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@agilo/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@agilo/plugin-sdk` from the local Agilo checkout into a `.agilo-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Agilo from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@agilo/plugin-sdk/ui`
