# Agilo Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Agilo's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Agilo app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Agilo HTTP APIs directly.
...
Agilo plugin design is based on the following assumptions:

1. Agilo is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Agilo business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Agilo core.
...
2. Let plugins add major capabilities without editing Agilo core.
...
4. Depend on project-local plugin folders such as `.agilo/plugins`.
...
The single Agilo deployment an operator installs and controls.

### 5.2 Company

A first-class Agilo business object inside the instance.
...
A trusted in-process extension loaded directly by Agilo core.
...
An installable instance-wide extension package loaded through the Agilo plugin runtime.
...
Agilo has two extension classes.
...
Agilo already has a concrete workspace model:
...
Plugins live under the Agilo instance directory.

Suggested layout:

- `~/.agilo/instances/default/plugins/package.json`
- `~/.agilo/instances/default/plugins/node_modules/`
- `~/.agilo/instances/default/plugins/.cache/`
- `~/.agilo/instances/default/data/plugins/<plugin-id>/`
...
Agilo should add CLI commands:

- `pnpm agilo plugin list`
- `pnpm agilo plugin install <package[@version]>`
- `pnpm agilo plugin uninstall <plugin-id>`
- `pnpm agilo plugin upgrade <plugin-id> [version]`
- `pnpm agilo plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@agilo/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface AgiloPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Agilo agents can use during runs.
...
- Agilo server starts one worker process per installed plugin
...
Receives one typed Agilo domain event.
...
export function definePlugin(definition: PluginDefinition): AgiloPlugin;
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@agilo/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@agilo/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@agilo/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@agilo/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@agilo/plugin-sdk/ui`
...
The `@agilo/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Agilo secret provider system rather than a plain text input
...
1. Core Agilo data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Agilo objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Agilo product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Agilo later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Agilo secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm agilo plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Agilo server. This is a normative requirement, not optional.
...
### 27.1 `@agilo/plugin-test-harness`
...
```ts
import { createTestHarness } from "@agilo/plugin-test-harness";
...
For developing a plugin against a running Agilo instance:

- The operator installs the plugin from a local path: `pnpm agilo plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-agilo-plugin`) that scaffolds:

- `package.json` with correct `agiloPlugin` keys
...
- `@agilo/plugin-workspace-files`
- `@agilo/plugin-terminal`
- `@agilo/plugin-git`
- `@agilo/plugin-linear`
- `@agilo/plugin-github-issues`
- `@agilo/plugin-grafana`
- `@agilo/plugin-runtime-processes`
- `@agilo/plugin-stripe`
...
- `@agilo/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@agilo/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@agilo/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@agilo/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@agilo/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@agilo/plugin-sdk/ui`
...
- `@agilo/plugin-test-harness`
- `create-agilo-plugin` starter template
...
Agilo should not implement a generic in-process hook bag modeled directly after local coding tools.

Agilo should implement:
...
That is the complete target design for the Agilo plugin system.
