...
Date: 2026-03-13  
Related discussion: https://github.com/agilo/agilo/discussions/449
...
- local runtime data from the default Agilo instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `agilo` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `agilo` skill currently tells agents to do this on essentially every heartbeat:
...
- Agilo discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Agilo skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Agilo-side skill changes land.
...
- `skills/agilo/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/agilo-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Agilo-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Agilo to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `agilo` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Agilo-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `agilo` skill rewrite
...
6. Rewrite `skills/agilo/SKILL.md` around delta-fetch behavior.
