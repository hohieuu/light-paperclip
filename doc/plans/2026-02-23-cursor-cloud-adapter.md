...
This document defines the V1 design for a Agilo adapter that integrates with
...
4. synthesized stdout events for Agilo UI/CLI
...
3. **Agent callback auth to Agilo** uses a bootstrap exchange flow (no long-lived Agilo key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Agilo endpoints, not full SKILL.md prompt injection.
...
- `agiloPublicUrl` (optional override; else `AGILO_PUBLIC_URL` env)
...
## Agilo Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `AGILO_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.agiloPublicUrl`
2. `process.env.AGILO_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Agilo credentials in prompt text.

Flow:

1. Before launch/follow-up, Agilo mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Agilo validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Agilo API calls.
...
1. Prompt includes a compact instruction to fetch skills from Agilo.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/agilo`
   - `GET /api/skills/agilo-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <agiloPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Agilo run context.
...
5. `agiloPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Agilo skill endpoints |
| Agilo auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Agilo context.
3. Add richer PR metadata surfacing in Agilo UI.
