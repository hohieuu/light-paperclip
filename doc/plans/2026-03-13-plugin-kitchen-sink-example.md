...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Agilo plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Agilo.
...
- package: `@agilo/plugin-kitchen-sink-example`
- plugin id: `agilo.kitchen-sink-example` or `agilo-kitchen-sink-example`

Recommendation: use `agilo-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Agilo event such as `issue.created`
...
Recommendation: default to a Agilo-local endpoint or a stable public echo endpoint to avoid flaky docs.
