...
Keep Agilo low-friction while making the mode model simpler and safer:
...
1. `pnpm agilo onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm agilo doctor` works flagless with mode-aware checks.
