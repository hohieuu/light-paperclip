...
Agents need API keys to authenticate with Agilo. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Agilo
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Agilo generates a token and passes it directly to the agent
...
`AGILO_API_URL`, `AGILO_AGENT_ID`, `AGILO_COMPANY_ID`. We need to
add a `AGILO_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Agilo CLI
...
```
agilo auth login
# Opens browser -> user confirms -> token stored at ~/.agilo/credentials
```
...
managed by the Agilo server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Agilo.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Agilo SKILL.md (or a link to it)
   - What information Agilo needs from the agent (e.g. webhook URL, adapter
...
5. Agilo stores the pending registration.
...
7. On approval, Agilo provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Agilo issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.agilo.tech/skills/agilo/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Agilo should send you heartbeats",
...
1. Generate a Agilo invite link for the company.
...
4. A Agilo company member approves the new agent.
5. Agilo begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `AGILO_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `agilo auth login` for developer-managed remote agents.                                      |
