...
- local mode that still feels instant (`npx agilo run` and go)
...
2. CLI command `pnpm agilo auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm agilo onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm agilo onboard`).
...
- expose key instance settings in API + CLI (`agilo configure` / `agilo onboard`)
...
7. `pnpm agilo onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
