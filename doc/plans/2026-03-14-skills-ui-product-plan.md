...
This document defines the product and UI plan for skill management in Agilo.
...
- Agilo company import/export can include skills as package content
...
3. The UI must always tell the truth about what Agilo knows:
   - desired state in Agilo
...
Agilo should treat skills at two scopes:
...
- created locally inside Agilo later
...
7. “Understand what Agilo knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Agilo-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Agilo did not install.

These should be clearly marked:

- external
- not managed by Agilo
...
- `desired_only`: Agilo wants it, adapter does not show it yet
- `external`: adapter has it but Agilo does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Agilo only tracks desired state
...
- Agilo can still track desired skills
...
- `.agilo.yaml` may add Agilo-specific fidelity, but should not replace the base shortname association model
...
- `.agilo.yaml` may add Agilo-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Agilo should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Agilo-specific.
...
That gives Agilo one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
