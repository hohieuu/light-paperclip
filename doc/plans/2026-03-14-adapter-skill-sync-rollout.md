...
This document defines the rollout plan for adapter-wide skill support in Agilo.
...
- Agilo stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Agilo currently has these adapters:
...
1. Desired skills live in Agilo for every adapter.
...
These adapters have a stable local skills directory that Agilo can read and manage.
...
These adapters do not have a meaningful Agilo-owned persistent install state.
...
- desired skills stored in Agilo
...
- runtime already injects Agilo skills into `~/.cursor/skills`
...
- runtime already injects Agilo skills into `~/.gemini/skills`
...
- runtime already injects Agilo skills into `~/.pi/agent/skills`
...
- Agilo can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Agilo skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Agilo may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Agilo skills that are clearly marked as Agilo-managed
...
- Agilo stores desired skills only
...
- avoid destructive wording unless Agilo can prove a skill is Agilo-managed
...
That gets Agilo from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
