...
Agilo's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Agilo's API, UI, and data model | Yes |
...
modules/
  observability/
    agilo.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@agilo/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@agilo/core";
...
```
1. Scan modules/ directory for agilo.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@agilo/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@agilo/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:agilo/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.agilo.tech/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm agilo store list                    # browse available modules and templates
pnpm agilo store install <module-id>     # install a module
pnpm agilo store import <template-id>    # import a company template
pnpm agilo store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Agilo instance | `server:started` |
...
4. **Module config** — `agilo.config.json` with per-module config, env var interpolation
...
10. **`@agilo/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
