...
Add a single storage subsystem for Agilo that supports:
...
- API serves attachment bytes through authenticated Agilo endpoints.
...
- Default local path is under instance root: `~/.agilo/instances/<instanceId>/data/storage`.
...
- `agilo onboard` writes a valid `storage` config block by default.
- `agilo configure --section storage` can switch between local and s3 modes.
...
- `agilo doctor` reports actionable storage status.
