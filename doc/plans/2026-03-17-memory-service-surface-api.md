# Agilo Memory Service Plan

## Goal

Define a Agilo memory service and surface API that can sit above multiple memory backends, while preserving Agilo's control-plane requirements:

- company scoping
- auditability
- provenance back to Agilo work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current Agilo architecture in:
...
Agilo should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Agilo plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. Agilo owns routing, provenance, and accounting

Providers should not decide how Agilo entities map to governance.

Agilo core should own:
...
The normalized Agilo scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through Agilo.

Agilo should log every operation, whether the provider is local or external.
...
## What Agilo Should Persist

Agilo should not mirror the full provider memory corpus into Postgres unless the provider is a Agilo-managed local provider.

Agilo core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, Agilo may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, Agilo may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, Agilo may capture selected issue comments or issue documents as memory sources.
...
Agilo should give agents both automatic recall and explicit tools, with simple guidance:
...
Agilo needs a first-class UI for memory, otherwise providers become black boxes.
...
This is important because a memory system that "works" but silently burns budget is not acceptable in Agilo.
...
- it matches Agilo's local-first posture
- it is inspectable
...
- Do we want Agilo-managed extraction pipelines at all, or should built-ins be the only place where Agilo owns extraction?
...
- Agilo owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Agilo a stable "memory service" without locking the product to one memory philosophy or one vendor.
