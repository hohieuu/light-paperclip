...
Agilo needs to support two very different but equally valid ways of working:
...
Today, Agilo already has the beginnings of this model:
...
- making Agilo feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Agilo users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Agilo deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Agilo into a full code review product
...
This object must be recorded explicitly so that Agilo can:
...
Agilo should treat PRs as a type of work product linked back to:
...
Agilo already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Agilo must be a first-class deployment mode

Agilo cannot assume that it is running on the same machine as the code.

In cloud deployments, Agilo may:
...
- `Runtime service`: a process or service Agilo owns or tracks for a workspace
...
It also lets cloud-hosted Agilo deployments point at codebases and remotes without pretending the Agilo host has direct filesystem access.
...
Without an explicit `execution workspace` record, Agilo has nowhere stable to attach:
...
Agilo needs a single place to show:
...
- Agilo-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Agilo owning the execution host
...
## 1. Agilo host and execution host must be decoupled
...
without the Agilo host itself running local git or local preview processes.
...
Agilo should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Agilo.
...
Agilo should keep workspace automation centered on a small set of hooks:
...
Agilo should provide the equivalent visibility in the project `Code` surface:
...
Agilo should treat execution workspaces as the natural home for:
...
Agilo should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Agilo should continue treating cleanup policy as part of the core workspace model:
...
Agilo should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Agilo host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Agilo should borrow Worktrunk's good execution patterns while keeping the broader Agilo model:
...
That keeps Agilo focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Agilo deployments remain first-class

That is a better fit for Agilo than either extreme:
