...
This document defines the next-stage plan for Agilo company import/export.

The core shift is:

- move from a Agilo-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Agilo.
...
Agilo already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Agilo
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `agilo.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.agilo.yaml` sidecar for high-fidelity Agilo-specific details
...
11. adapter-aware skill sync surfaces so Agilo can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Agilo at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Agilo sidecar.
...
Agilo must not redefine `SKILL.md`.
...
- Agilo-specific fidelity lives in `.agilo.yaml`
- Agilo may resolve and install `SKILL.md` packages, but it must not require a Agilo-only skill format
...
### 5.4 Base Package Vs Agilo Extension
...
- base package:
  ...
  - no Agilo-only runtime fields by default
- Agilo extension:
  - `.agilo.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Agilo tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`agilo.manifest.json` is not part of the future package direction.
...
Agilo import/export should support these entity kinds:
...
- Agilo can treat “this set of agents came from team package X” as the imported-team model
...
Agilo should support source references in package metadata with:
...
Agilo should define a new adapter surface area around skills:
...
Planned Agilo behavior:

- if an adapter supports read, Agilo should show current skills in the UI
- if an adapter supports write, Agilo should let the user enable/disable imported skills
- if an adapter supports sync, Agilo should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.agilo.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.agilo.yaml` alongside the base package
...
Agilo should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `agilo company export <company-id> --out <path>`
- `agilo company import <path-or-url> --dry-run`
- `agilo company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Agilo storage, or only referenced at import time?
...
3. Should Agilo support direct local folder selection in the web UI, or keep that CLI-only initially?
