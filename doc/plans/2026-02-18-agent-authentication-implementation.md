...
- New config/env settings:
  - `AGILO_AGENT_JWT_SECRET`
  - `AGILO_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `AGILO_AGENT_JWT_ISSUER` (default: `agilo`)
  - `AGILO_AGENT_JWT_AUDIENCE` (default: `agilo-api`)
...
1. In:
   ...
   inject `AGILO_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `AGILO_API_KEY`, do not overwrite it.
- Continue injecting:
  - `AGILO_AGENT_ID`
  - `AGILO_COMPANY_ID`
  - `AGILO_API_URL`
...
- Local adapters authenticate without manual `AGILO_API_KEY` config.
