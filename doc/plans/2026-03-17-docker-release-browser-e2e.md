...
Today release smoke testing for published Agilo packages is manual and shell-driven:
...
- runs `agilo@${AGILOAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@agilo.local`
- password: `agilo-smoke-password`
...
- the suite passes locally against both:
  - `AGILOAI_VERSION=canary`
  - `AGILOAI_VERSION=latest`
...
1. A published `agilo@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `agilo@latest` Docker install can be smoke-tested by Playwright in CI.
