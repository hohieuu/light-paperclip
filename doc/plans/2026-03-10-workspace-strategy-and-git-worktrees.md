...
- the workflow should reuse the same Agilo instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Agilo:

- some operators use Agilo to manage Agilo and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Agilo
- Claude and Codex expose different built-in affordances, so Agilo should not overfit to one tool

## Core Product Decision

Agilo should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Agilo
...
- Agilo should not assume that every adapter can see or use a host filesystem path directly
...
If Agilo makes worktrees agent-first, it will blur:
...
- Agilo core resolves and records execution workspace state
...
That gives Agilo room to support:
...
- Agilo handles workspace runtime services under the hood using project/workspace policy
...
Once Agilo is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Agilo cannot infer
...
- when issue moves to done, should Agilo auto-commit?
- should it auto-push?
...
Agilo already has the right foundation for a project-first model.
...
For server-spawned local adapters, Agilo already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Agilo should mirror directly

Implication:

- for `codex_local`, Agilo should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Agilo should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Agilo should separate:
...
Agilo should be able to express intentions such as:
...
These are not specific to Agilo. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".agilo/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Agilo-specific `devServer` flag.
...
Agilo should distinguish between:
...
Agilo should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Agilo should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Agilo
...
For runtime services specifically, Agilo should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Agilo-only dev-server flags
...
3. Inject resolved execution workspace details into `context.agiloWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Agilo instance can be reused through env/config injection when appropriate
...
- Agilo can decide whether to reuse or start a fresh service deterministically
...
3. Test against Agilo-on-Agilo workflows first.
...
7. Workspace runtime services are modeled generically, not as Agilo-specific dev-server toggles.
