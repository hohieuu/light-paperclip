...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/agilo-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `AGILO_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `AGILO_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Agilo may create host-local worktrees, processes, and paths
- `cloud`
  - Agilo should assume no durable host-local execution workspace management
...
- use `AGILO_EXECUTION_TOPOLOGY` as a defaulting guardrail
