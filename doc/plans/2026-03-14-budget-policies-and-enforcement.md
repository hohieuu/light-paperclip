...
Agilo already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Agilo should implement next.

## Product Goals

Agilo should let operators:
...
For the next implementation pass, Agilo should enforce these defaults:
...
Agilo should separate subscription-included usage from billed spend:
...
Agilo should have two threshold classes:
...
Agilo must know that a project is budget-paused and enforce it.
...
Agilo must also block execution before new work starts.
