...
Maintainer runbook for shipping Agilo across npm, GitHub, and the website-facing changelog surface.
...
Agilo uses calendar versions that still fit semver syntax:
...
2. **npm** — `agilo` and public workspace packages are published
...
```bash
npx agilo@canary onboard
# or
npx agilo@canary onboard --data-dir "$(mktemp -d /tmp/agilo-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/agilo/agilo/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Agilo. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
AGILOAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
AGILOAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary AGILOAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable AGILOAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f agilo_version=canary
gh workflow run release-smoke.yml -f agilo_version=latest
```

Minimum checks:

- `npx agilo@canary onboard` installs
