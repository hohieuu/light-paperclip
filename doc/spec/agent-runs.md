...
This spec defines how Agilo actually runs agents while staying runtime-agnostic.
...
1. Agilo is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Agilo manages lifecycle, and on exit Agilo parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `agilo.skill` (shared Agilo skill text block)
...
2. Preferred transport is env vars (`AGILO_*`) injected at runtime.
