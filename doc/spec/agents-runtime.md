...
For `claude_local` and `codex_local`, Agilo assumes the CLI is already installed and authenticated on the host machine.
...
Agilo stores resumable session state per `(agent, taskKey, adapterType)`.
