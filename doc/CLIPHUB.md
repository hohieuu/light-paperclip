...
ClipHub is the public registry where people share, discover, and download Agilo company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Agilo what a package registry is to a programming language. Agilo already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Agilo — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Agilo instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Agilo spec. It contains:
...
**Install (fresh start):**
```
agilo install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Agilo instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Agilo, export your company as a template, then publish:

```
agilo export --template my-company
agilo publish cliphub my-company
```
...
ClipHub is a **separate service** from Agilo itself. Agilo is self-hosted; ClipHub is a hosted registry that Agilo instances talk to.
...
| **Agilo CLI** | `agilo install`, `agilo publish`, `agilo cliphub sync` — built into Agilo |
| **Agilo UI** | "Browse ClipHub" panel in the Agilo web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Agilo) |
| Backend | TypeScript + Hono (consistent with Agilo) |
| Database | PostgreSQL |
...
4. Run `agilo install cliphub:acme/lean-saas-shop`
5. Agilo creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Agilo until it works well
2. Export: `agilo export --template my-agency`
3. Publish: `agilo publish cliphub my-agency`
...
3. Install just that agent: `agilo install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Agilo

ClipHub is **not required** to use Agilo. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Agilo what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `agilo install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Agilo web UI
- [ ] `agilo cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Agilo's own philosophy)
