...
Low-level reference for how Agilo packages are prepared and published to npm.
...
Agilo no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `agilo`, imports code from workspace packages such as:
...
### Manual first publish for `@agilo/ui`
...
npm view @agilo/ui version

# make sure the dist payload is fresh
pnpm --filter @agilo/ui build
...
- If `npm view @agilo/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@agilo/ui`, the `agilo` npm organization must exist and the publisher must be a member with permission to publish to that scope.
