...
description: >
  Technical guide for creating a new Agilo agent adapter. Use when building
...
# Creating a Agilo Agent Adapter

An adapter bridges Agilo's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `agilo run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@agilo/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@agilo/adapter-utils` (types) or `@agilo/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell Agilo to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including Agilo agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@agilo/adapter-utils/server-utils`)
2. **Build environment** — call `buildAgiloEnv(agent)` then layer in `AGILO_RUN_ID`, context vars (`AGILO_TASK_ID`, `AGILO_WAKE_REASON`, `AGILO_WAKE_COMMENT_ID`, `AGILO_APPROVAL_ID`, `AGILO_APPROVAL_STATUS`, `AGILO_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `AGILO_AGENT_ID` | `agent.id` |
| `AGILO_COMPANY_ID` | `agent.companyId` |
| `AGILO_API_URL` | Server's own URL |
| `AGILO_RUN_ID` | Current run id |
| `AGILO_TASK_ID` | `context.taskId` or `context.issueId` |
| `AGILO_WAKE_REASON` | `context.wakeReason` |
| `AGILO_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `AGILO_APPROVAL_ID` | `context.approvalId` |
| `AGILO_APPROVAL_STATUS` | `context.approvalStatus` |
| `AGILO_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `AGILO_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `agilo run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@agilo/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@agilo/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use Agilo API
...
import { parseMyAgentStdoutLine } from "@agilo/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@agilo/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@agilo/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so Agilo wipes the stale session
...
Import from `@agilo/adapter-utils/server-utils`:
...
| `buildAgiloEnv(agent)` | Standard `AGILO_*` env vars |
...
- Package name: `@agilo/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your Agilo work."`
...
### Agilo Skills Injection

Agilo ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `agilo` API skill and the `agilo-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with Agilo internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("agilo-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(AGILO_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(AGILO_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks Agilo skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The Agilo skills (like `agilo` and `agilo-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the Agilo API to report status), use explicit instructions in the prompt template: "Use the agilo skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between Agilo's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `AGILO_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the Agilo API and GitHub should not have access to arbitrary hosts.
