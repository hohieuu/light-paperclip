 ...
 *    (`~/.agilo/plugins/`) and `node_modules` for packages matching
 *    the `agilo-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Agilo plugins.
 * Packages matching this pattern are considered Agilo plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "agilo-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: AgiloPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.agilo/plugins/ local directory
  | "npm"               // npm packages matching agilo-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: AgiloPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the agilo-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "agilo-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AgiloPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Agilo plugins installed as npm packages in the current
   * Node.js environment matching the "agilo-plugin-*" naming convention.
   *
   ...
   * the "agiloPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Agilo plugin.
   * Throws if the package is a Agilo plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AgiloPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AgiloPluginManifestV1;
    newManifest: AgiloPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Agilo plugin naming convention.
 * Accepts both the "agilo-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @agilo/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "agiloPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const agiloPlugin = pkgJson["agiloPlugin"];
  if (
    agiloPlugin !== null &&
    typeof agiloPlugin === "object" &&
    !Array.isArray(agiloPlugin)
  ) {
    const manifestRelPath = (agiloPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: AgiloPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAgiloVersion;
}
 ...
 *   packageName: "agilo-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AgiloPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AgiloPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Agilo plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a Agilo plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAgiloPlugin = "agiloPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAgiloPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AgiloPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: AgiloPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @agilo/shared exports). Run those workers through
