...
import { companySkills } from "@agilo/db";
import { readAgiloSkillSyncPreference, writeAgiloSkillSyncPreference } from "@agilo/adapter-utils/server-utils";
import type { AgiloSkillEntry } from "@agilo/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@agilo/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveAgiloInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("agilo/agilo/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveAgiloInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "agilo_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Agilo skills are read-only.",
      sourceLabel: "Agilo bundled",
      sourceBadge: "agilo",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "agilo_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Agilo skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "agilo_bundled"
        && incomingKind === "github"
        && incomingOwner === "agilo"
        && incomingRepo === "agilo"
      ) {
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeAgiloSkillSyncPreference(config, filtered),
        });
      }
    }
