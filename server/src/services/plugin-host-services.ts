...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@agilo/db";
 ...
 *
 * This implementation delegates to the core Agilo domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@agilo/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
