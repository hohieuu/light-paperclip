 ...
 */
import { pluginManifestV1Schema } from "@agilo/shared";
import type { AgiloPluginManifestV1 } from "@agilo/shared";
import { PLUGIN_API_VERSION } from "@agilo/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AgiloPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AgiloPluginManifestV1,
        };
      }
