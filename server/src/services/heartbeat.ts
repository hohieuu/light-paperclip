...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@agilo/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_agiloWakeContext";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      agiloRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.agiloWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.agiloRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.agiloRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.agiloSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.agiloSessionRotationReason = sessionCompaction.reason;
      context.agiloPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.agiloSessionHandoffMarkdown;
      delete context.agiloSessionRotationReason;
      delete context.agiloPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.agiloRuntimeServices = runtimeServices;
        context.agiloRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[agilo] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected AGILO_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.agiloRuntimeServices = combinedRuntimeServices;
        context.agiloRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[agilo] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
