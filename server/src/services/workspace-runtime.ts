...
import type { AdapterRuntimeServiceReport } from "@agilo/adapter-utils";
import type { Db } from "@agilo/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@agilo/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AGILO_WORKSPACE_CWD = input.worktreePath;
  env.AGILO_WORKSPACE_PATH = input.worktreePath;
  env.AGILO_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AGILO_WORKSPACE_BRANCH = input.branchName;
  env.AGILO_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AGILO_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AGILO_WORKSPACE_SOURCE = input.base.source;
  env.AGILO_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AGILO_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AGILO_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AGILO_PROJECT_ID = input.base.projectId ?? "";
  env.AGILO_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AGILO_AGENT_ID = input.agent.id ?? "";
  env.AGILO_AGENT_NAME = input.agent.name;
  env.AGILO_COMPANY_ID = input.agent.companyId;
  env.AGILO_ISSUE_ID = input.issue?.id ?? "";
  env.AGILO_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AGILO_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AGILO_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AGILO_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AGILO_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AGILO_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AGILO_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AGILO_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AGILO_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AGILO_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AGILO_PROJECT_ID = input.workspace.projectId ?? "";
  env.AGILO_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AGILO_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".agilo", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Agilo", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
