const FAVICON_BLOCK_START = "<!-- AGILO_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- AGILO_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- AGILO_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AGILO_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AGILO_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AGILO_WORKTREE_NAME) ?? nonEmpty(env.AGILO_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AGILO_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="agilo-worktree-enabled" content="true" />',
    `<meta name="agilo-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="agilo-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="agilo-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
