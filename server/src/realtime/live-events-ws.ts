...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@agilo/db";
import type { DeploymentMode } from "@agilo/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  agiloUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).agiloUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.agiloUpgradeContext = context;
