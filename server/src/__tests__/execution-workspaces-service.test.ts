...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Agilo Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@agilo.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("agilo-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `agilo-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "agilo-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "agilo-close-check"]);
