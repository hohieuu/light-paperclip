...
import { ensureCursorSkillsInjected } from "@agilo/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Agilo skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("agilo-cursor-skills-src-");
    const skillsHome = await makeTempDir("agilo-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "agilo");
    await createSkillDir(skillsDir, "agilo-create-agent");
    ...
    const injectedA = path.join(skillsHome, "agilo");
    const injectedB = path.join(skillsHome, "agilo-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "agilo")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "agilo-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "agilo"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "agilo-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("agilo-cursor-preserve-src-");
    const skillsHome = await makeTempDir("agilo-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "agilo");
    ...
    expect((await fs.lstat(path.join(skillsHome, "agilo-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("agilo-cursor-fail-src-");
    const skillsHome = await makeTempDir("agilo-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
