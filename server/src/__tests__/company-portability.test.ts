...
import type { CompanyPortabilityFileEntry } from "@agilo/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "agilo",
        name: "agilo",
        description: "Agilo coordination skill",
        markdown: "---\nname: agilo\ndescription: Agilo coordination skill\n---\n\n# Agilo\n",
        sourceType: "github",
        sourceLocator: "https://github.com/agilo/agilo/tree/master/skills/agilo",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/agilo/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "agilo",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/agilo/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "agilo",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Agilo extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Agilo"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${agiloKey}"`);
    ...
    expect(extension).not.toContain("agiloSkillSync");
  ...
  it("exports default sidebar order into the Agilo extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".agilo.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/agilo/agilo/agilo/SKILL.md"])).toContain("# Agilo");
    expect(asTextFile(exported.files["skills/agilo/agilo/agilo/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agilo/agilo/agilo/references/api.md"])).toContain("# API");
  });
  ...
  it("exports the company logo into images/ and references it from .agilo.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".agilo.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        ...
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/agilo/agilo/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "agilo_bundled",
          owner: "agilo",
          repo: "agilo",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/agilo/agilo/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agilo/agilo/release-changelog/SKILL.md"])).toContain("agilo/agilo/release-changelog");
  });
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/agilo",
            repoUrl: "https://github.com/agilo/agilo.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .agilo-tmp",
            remoteProvider: null,
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/agilo/agilo.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".agilo.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/agilo/agilo.git"');
    expect(extension).toContain('projectWorkspaceKey: "agilo"');
  ...
  it("imports a vendor-neutral package without .agilo.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Agilo");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".agilo.yaml"] = `${exported.files[".agilo.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
