...
describe("agent instructions service", () => {
  const originalAgiloHome = process.env.AGILO_HOME;
  const originalAgiloInstanceId = process.env.AGILO_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalAgiloHome === undefined) delete process.env.AGILO_HOME;
    else process.env.AGILO_HOME = originalAgiloHome;
    if (originalAgiloInstanceId === undefined) delete process.env.AGILO_INSTANCE_ID;
    else process.env.AGILO_INSTANCE_ID = originalAgiloInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-home-");
    const externalRoot = await makeTempDir("agilo-agent-instructions-external-");
    cleanupDirs.add(agiloHome);
    cleanupDirs.add(externalRoot);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("agilo-agent-instructions-new-external-");
    cleanupDirs.add(agiloHome);
    cleanupDirs.add(externalRoot);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("agilo-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-recover-");
    cleanupDirs.add(agiloHome);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("agilo-agent-instructions-stale-root-");
    cleanupDirs.add(agiloHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("agilo-agent-instructions-heal-write-stale-");
    cleanupDirs.add(agiloHome);
    cleanupDirs.add(staleRoot);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("agilo-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(agiloHome);
    cleanupDirs.add(staleRoot);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const agiloHome = await makeTempDir("agilo-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("agilo-agent-instructions-partial-root-");
    cleanupDirs.add(agiloHome);
    cleanupDirs.add(staleRoot);
    process.env.AGILO_HOME = agiloHome;
    process.env.AGILO_INSTANCE_ID = "test-instance";
