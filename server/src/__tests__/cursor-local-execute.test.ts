...
import { execute } from "@agilo/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects agilo env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the agilo heartbeat.");
      ...
      expect(capture.prompt).toContain("Agilo runtime note:");
      expect(capture.prompt).toContain("AGILO_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "agilo-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const agiloDir = await createSkillDir(runtimeSkillsRoot, "agilo");
