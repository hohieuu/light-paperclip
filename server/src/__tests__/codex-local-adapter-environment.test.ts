...
import { testEnvironment } from "@agilo/adapter-codex-local/server";
...
describe("codex_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `agilo-codex-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("emits codex_native_auth_present when ~/.codex/auth.json exists and OPENAI_API_KEY is unset", async () => {
    const root = path.join(
      os.tmpdir(),
      `agilo-codex-auth-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
  ...
  it("emits codex_openai_api_key_missing when neither env var nor native auth exists", async () => {
    const root = path.join(
      os.tmpdir(),
      `agilo-codex-noauth-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    const codexHome = path.join(root, ".codex");
    const cwd = path.join(root, "workspace");
  ...
  itWindows("runs the hello probe when Codex is available via a Windows .cmd wrapper", async () => {
    const root = path.join(
      os.tmpdir(),
      `agilo-codex-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
