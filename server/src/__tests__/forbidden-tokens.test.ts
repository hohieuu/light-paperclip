...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "agilo", LOGNAME: "agilo", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "agilo" }),
      },
    );

    expect(tokens).toEqual(["agilo", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "agilo" }, {
        userInfo: () => ({ username: "agilo" }),
      });

      expect(tokens).toEqual(["agilo", "custom-token"]);
    } finally {
