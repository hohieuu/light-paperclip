...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@agilo/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@agilo/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use agilo skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use agilo skill**" },
    ]);
  });
  ...
  it("parses command execution and file changes", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/agilouser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
    ...
    ).toEqual([
      {
        kind: "system",
        ts,
        text: "file changes: update /Users/agilouser/project/ui/src/pages/AgentDetail.tsx",
      },
    ]);
  });
