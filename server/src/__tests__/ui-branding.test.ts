...
<head>
    <!-- AGILO_RUNTIME_BRANDING_START -->
    <!-- AGILO_RUNTIME_BRANDING_END -->
    <!-- AGILO_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from AGILO_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ AGILO_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AGILO_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AGILO_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      AGILO_IN_WORKTREE: "true",
      AGILO_WORKTREE_NAME: "agilo-pr-432",
      AGILO_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("agilo-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="agilo-worktree-name"');
    expect(meta).toContain('content="agilo-pr-432"');
    expect(meta).toContain('name="agilo-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="agilo-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="agilo-worktree-name"');
  });
});
