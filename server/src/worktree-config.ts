...
import type { AgiloConfig } from "@agilo/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Agilo worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.AGILO_IN_WORKTREE !== "true") return null;

  const configPath = resolveAgiloConfigPath(overrideConfigPath);
  const envPath = resolveAgiloEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.AGILO_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.AGILO_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.AGILO_HOME) ??
      nonEmpty(env.AGILO_WORKTREES_DIR) ??
      "~/.agilo-worktrees",
  );
...
function writeConfigFile(configPath: string, config: AgiloConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".agilo", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as AgiloConfig;
...
): { config: AgiloConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.AGILO_HOME = context.homeDir;
  process.env.AGILO_INSTANCE_ID = context.instanceId;
  process.env.AGILO_CONFIG = context.configPath;
  process.env.AGILO_CONTEXT = context.contextPath;
  process.env.AGILO_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgiloConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    AGILO_HOME: context.homeDir,
    AGILO_INSTANCE_ID: context.instanceId,
    AGILO_CONFIG: context.configPath,
    AGILO_CONTEXT: context.contextPath,
    AGILO_IN_WORKTREE: "true",
    AGILO_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: AgiloConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgiloConfig;
  } catch {
    return;
  }
