...
import { agents as agentsTable, companies, heartbeatRuns } from "@agilo/db";
...
import { runClaudeLogin } from "@agilo/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@agilo/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@agilo/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@agilo/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@agilo/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeAgiloSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readAgiloSkillSyncPreference(
