...
function jwtConfig() {
  const secret = process.env.AGILO_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.AGILO_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.AGILO_AGENT_JWT_ISSUER ?? "agilo",
    audience: process.env.AGILO_AGENT_JWT_AUDIENCE ?? "agilo-api",
  };
}
