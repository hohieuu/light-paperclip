...
import { resolveAgiloConfigPath, resolveAgiloEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.AGILO_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.AGILO_AGENT_JWT_SECRET === "string" ? parsed.AGILO_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm agilo onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveAgiloEnvPath();
