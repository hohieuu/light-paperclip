---
name: agilo-create-plugin
description: >
  Create new Agilo plugins with the current alpha SDK/runtime. Use when
...
# Create a Agilo Plugin

Use this skill when the task is to create, scaffold, or document a Agilo plugin.
...
node packages/plugins/create-agilo-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Agilo repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.agilo-sdk/`:

```bash
pnpm --filter @agilo/create-agilo-plugin build
node packages/plugins/create-agilo-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Agilo from an absolute local path during development
