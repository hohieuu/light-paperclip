...
Agilo supports two runtime modes with different security profiles.
...
```sh
pnpm agilo allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Agilo emits a one-time claim URL at startup:
...
```sh
AGILO_DEPLOYMENT_MODE=authenticated pnpm agilo run
```
