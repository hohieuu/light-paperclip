...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Agilo.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.agilo.yaml`
...
- Agilo must not require extra top-level fields for skill validity
- Agilo-specific extensions must live under `metadata.agilo` or `metadata.sources`
...
For Agilo, the preferred fidelity extension is:
...
- issue/task Agilo-only metadata
...
- Agilo tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Agilo shape:
...
Additional rules for Agilo exporters:
...
- omit empty and default-valued Agilo fields when possible
...
- Agilo exporters should emit `.agilo.yaml` as a sidecar by default
...
Agilo can map this spec to its runtime model like this:
...
- Agilo extension:
  - `.agilo.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Agilo-specific fidelity

Inline Agilo-only metadata that must live inside a shared markdown file should use:

- `metadata.agilo`

That keeps the base format broader than Agilo.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Agilo.

## 20. Cutover

Agilo should cut over to this markdown-first package model as the primary portability format.

`agilo.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Agilo, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
