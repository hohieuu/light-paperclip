...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Agilo-Signature` + `X-Agilo-Timestamp` header pair matching the trigger's signing mode.
