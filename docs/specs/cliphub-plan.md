# ClipHub: Marketplace for Agilo Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Agilo configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Agilo-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Agilo company (agents, projects, governance) |
...
A complete Agilo company configuration:
...
Single-agent configurations designed to plug into a Agilo org:
...
Portable skill files that any Agilo agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Agilo's skill loading system
...
interface Listing {
  ...
  agiloVersionMin: string;     // Minimum Agilo version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Agilo company
...
| `POST` | `/api/listings/:id/install` | Install to Agilo company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Agilo company (or create new)
  → ClipHub API calls Agilo API to:
  ...
  → Redirect to Agilo dashboard with new team running
```
...
### 5.3 Creator: Export from Agilo → Publish

```
Running Agilo company → "Export as Blueprint" (CLI or UI)
  → Agilo exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Agilo brand blue/purple)
...
| Install flow | One-click deploy to Agilo company |
...
{
  "targetCompanyId": "uuid",         // Existing Agilo company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `agilo-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Agilo
- **Backend**: Node.js API (or extend Agilo server)
- **Database**: Postgres (can share Agilo's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Agilo auth (or OAuth2)

### 9.2 Integration with Agilo

ClipHub can be:
- **Option A**: A separate app that calls Agilo's API to install blueprints
- **Option B**: A built-in section of the Agilo UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Agilo UI and API.
...
- [ ] Install flow: blueprint → Agilo company
