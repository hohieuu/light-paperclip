...
- Running a Python script that calls the Agilo API
...
1. Agilo spawns the configured command as a child process
2. Standard Agilo environment variables are injected (`AGILO_AGENT_ID`, `AGILO_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Agilo API and perform work.
