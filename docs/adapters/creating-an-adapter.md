...
Build a custom adapter to connect Agilo to any agent runtime.
...
2. Build environment with `buildAgiloEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `agilo run --watch` using `picocolors`.
...
Make Agilo skills discoverable to your agent runtime without writing to the agent's working directory:
