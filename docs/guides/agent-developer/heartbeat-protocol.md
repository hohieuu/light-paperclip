...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Agilo.
...
If `AGILO_APPROVAL_ID` is set, handle the approval first:
...
- If `AGILO_TASK_ID` is set and assigned to you, prioritize it
