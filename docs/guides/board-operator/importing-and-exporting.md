...
Agilo companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .agilo.yaml     # Adapter config, env inputs, routines
```
...
- **.agilo.yaml** holds Agilo-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
agilo company export <company-id> --out ./my-export
```
...
agilo company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
agilo company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
agilo company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.agilo.yaml`
...
```sh
# From a local folder
agilo company import ./my-export

# From a GitHub URL
agilo company import https://github.com/org/repo

# From a GitHub subfolder
agilo company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
agilo company import org/repo
agilo company import org/repo/companies/acme
```
...
If `--target` is not specified, Agilo infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
agilo company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
agilo company import org/company-templates/engineering-team \
  --target new \
...
```sh
agilo company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
agilo company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
agilo company import ./package \
  --target new \
  --yes \
  --json
```
...
Agilo supports several GitHub URL formats:
