...
Add first-class **documents** to Agilo as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Agilo workflow/docs.
...
Update the Agilo agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/agilo/SKILL.md`
